...
@section('content')
    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Indisipliner / Guru /</span> Rekapitulasi Pelanggaran</h4>
    ...
    <div class="card mb-4">
        <div class="card-header">
            <h5 class="card-title mb-0"><i class="bx bx-search-alt me-2"></i>Cari Rapor Pelanggaran Guru</h5>
            </div>
        <div class="card-body">
            <form action="{{ route('admin.indisipliner.guru.rekapitulasi.index') }}" method="GET">
                <div class="row g-3 align-items-end">
                    <div class="col-md-10">
                        <label for="nip" class="form-label">Pilih Guru/GTK</label>
                        <select name="nip" id="nip" class="form-select" required>
                            <option value="">Ketik atau pilih nama guru...</option>
                            {{-- Ini adalah variabel $guruList dari controller --}}
                            @foreach ($guruList as $g)
                                <option value="{{ $g->nip }}" {{ request('nip') == $g->nip ? 'selected' : '' }}>
                                    {{ $g->nama }} (NIP: {{ $g->nip }})
                                    </option>
                            @endforeach
                            </select>
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0">
                    <i class="bx bx-user-circle me-2"></i>Rapor Pelanggaran:
            ...
            <div class="card-body">
                <div class="row">
                    {{-- Kolom Kiri: Informasi Guru --}}
                    <div class="col-md-7 border-end">
                        <div class="d-flex align-items-center mb-4">
                            <div class="flex-shrink-0 me-3">
                                <img
                                    src="https://placehold.co/100x100/17a2b8/white?text={{ substr($guru->nama, 0, 1) }}"
                                    alt="Avatar" class="d-block rounded-circle" height="100" width="100">
                                </div>
                            <div class="flex-grow-1">
                                <h5 class="mb-1">{{ $guru->nama }}</h5>
                                <p class="text-muted mb-0">
                                    {{ $guru->jenis_pegawai ?? 'Tenaga Pendidik/Kependidikan' }}</p>
                                </div>
                            </div>
                    ...
                    <div class="col-md-5 d-flex flex-column justify-content-center align-items-center p-4">
                        ...
                        @if ($sanksiAktif)
                            <div class="text-center mt-3">
                                ...
                                <span
                                    class="badge bg-label-danger fs-6">{{ $sanksiAktif->nama }}</span>
                                </div>
                        @else
                            <div class="text-center mt-3">
                        ...
                        <tbody>
                            @forelse($pelanggaranGuru as $key => $pelanggaran)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>
                                        {{ \Carbon\Carbon::parse($pelanggaran->tanggal)->format('d M Y') }}</td>
                                    <td style="white-space: normal;">
                                        {{ $pelanggaran->detailPoinGtk->nama ?? '-' }}</td>
                                    <td><span
                                            class="badge bg-danger rounded-pill">{{ $pelanggaran->poin }}</span></td>
                                    </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center py-4">
                                        <i class="bx bx-check-circle bx-lg text-success"></i>
                                        <p class="text-muted mt-2 mb-0">Guru ini tidak memiliki
                                            riwayat pelanggaran.</p>
                                        </td>
                                    </tr>
                            @endforelse
                            </tbody>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            // Inisialisasi Select2 dummy agar tampilan pesan awal muncul
            // Jika Anda ingin Select2 di dalam pesan "Mulai Pencarian" juga terlihat,
            // tambahkan inisialisasi untuk ID dummy.
            $('#nip_dummy').select2({
                theme: "bootstrap-5",
                placeholder: "Ketik atau pilih nama guru...",
